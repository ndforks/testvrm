...
class Projectslist extends Admin_Controller
{
    ...
    public function status($status = 'active', $page = 0)
    {
        ...
        $this->layout->buffer('content', 'projectlist/index');
        $this->layout->render();
    }
    ...
    public function form($id = null)
    {
        ...
        if ($this->mdl_clients->run_validation()) {
            ...
            if ($result !== true) {
                ...
                redirect('projectlist/form/' . $id);
                return;
            } else {
                redirect('projectlist/view/' . $id);
            }
        }
        ...
        $this->layout->buffer('content', 'projectlist/form');
        $this->layout->render();
    }
    ...
    {
        $this->load->model('projectlist/mdl_client_notes');
        $this->load->model('invoices/mdl_invoices');
        $this->load->model('quotes/mdl_quotes');
		$this->load->model('projquotes/mdl_quotes');
        ...
        $this->layout->set(
            array(
				...
				'projquotes' => $this->mdl_quotes->by_client($client_id)->limit(20)->get()->result(),
        ...
        $this->layout->buffer(
            array(
                ...
                array(
                    'quote_table',
                    'projquotes/partial_quote_table'
                ),
				array(
                    'quote_table',
